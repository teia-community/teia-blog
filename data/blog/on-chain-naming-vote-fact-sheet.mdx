...
If you sync your wallet to our voting tool [vote.hencommunity.quest](https://vote.hencommunity.quest/), you can vote for your favourite name (only 1 vote) from a shortlist of 10 names (link to the vote below).

There will be 2 on-chain voting rounds: The first round (running for 7 days) will be on a shortlist of 10 name options, the second (run-off) round will start after the first round and include the top 2 voted on names from round 1 and last for 5 days.
Note that only active hicetnunc wallets created before Jan16 2022 that are not banned are eligible to vote in order to prevent a dilution of the vote due to the creation of new accounts to gain voting power. 
You can change your vote while the voting round is ongoing by voting again (you will still only have 1 vote).

You can find a little background info and discription for all the voting options in this [PDF document by the HENC branding team](https://drive.google.com/file/d/1op6K1FfwlCadErYg4ZMPEdU0zUruHVyA/view?usp=sharing)


**Voting round 1** 
will run from 2022-01-18 00:00 UTC until 2022-01-25 00:00 UTC
[Click here to participate in voting round 1](https://vote.hencommunity.quest/)
...
**For more details** about this process and the people that keep this platform running, read the FAQ section below,  follow our current [twitter account](https://twitter.com/hen_community), vist [our blog (currently under construction)](https://hencommunity.quest/) or join the [discord](https://discord.gg/cXGymBsE), or check out these recaps of the discussions
...
Henvote is an on-chain voting tool developed during the hicathon specifically for straight-forward on-chain voting by hicetnunc wallets. This tool will be used for voting purposes around this community until we have a fully formed DAO structure implemented. you can find more information on [https://vote.hencommunity.quest](https://vote.hencommunity.quest/)

> 7. What weight does my vote have? Why can only wallets participate that were created before 11/11/2021?

For this naming vote, every wallet will have the same voting weight (1 vote). All wallets created and active on hen before 01/16/202 and not banned on hicetnunc are eligible to 1 vote. The cutoff date of 01/16/2022 (a few days before the vote started) is a straight-forward filter to prevent participants from creating new (multiple) accounts to get more voting weight.
