...
draft: false
summary: 'Everything you need to know about Teias DAO Token distribution'
bibliography: references-data.bib
authors: ['jagracar', 'merchantcoppola', 'denscimonk', 'badlament']
---

_Everything you need to know about Teias DAO Token distribution_
...
For Phase 02, we want to move to a truly community-owned DAO that is defined by token ownership for the most part. For this, we need to start distributing the governance tokens.
...
So, when will the Teia DAO governance tokens be distributed among the community? A few final steps and decisions need to be made within the team and the wider community, and then it's finally time to claim your tokens. Please keep an eye on Teia's Twitter account and blog to not miss out on updates and announcements!

- Second half of May: Community vote on total supply.
- After the vote: Adjusting contributors sheet, preparing data and token claim page for distribution, agreeing on the length of the distribution period.
- Early to mid-June: Token distribution period starts. Teians can claim their Teia DAO tokens.
- After the distribution, we can start using the DAO contracts for voting and decisionmaking.
...
- **Contribution** to Teia and Hic et Nunc defined by many different distribution parameters as layed out below
- **hDAO conversion** - a 1:1 hDAO for teia DAO Token reward.

650k Tokens are allocated as hDAO rewards, 350k Tokens are reserved for the Teia treasury (fututre distribution) adding up to 1 Million, the remaining supply is split up accoding to the following paramters:

### Activity/Contribution Parameters:
...
You can find a more detailed explanation of the activity/contribution parameters in [the list below](blog.teia.art/fact-sheet-token-drop#what-do-the-parameters-on-the-distribution-model-sheets-mean-exactly)
...
There is a fixed overall supply of 650k hDAO tokens, we want to offer a 1:1 conversion to Teia DAO tokens. This means that the total supply will determine what percentage of the overall tokens will be distributed for activity and contribution. The community will be able to vote on the overall supply of Teia tokens between the following options for total supply.
...
In a few days, there will be a community vote that will determine how many Teia DAO tokens will be minted in total. Teia DAO Tokens will have a fixed supply, meaning there won't be additional tokens issued for this token in the future. As mentioned above, the main difference between the different supplies will be the percentage of Tokens allocated for the hDAO rewards.
...
You can cast your vote here on vote.teia.art: [link will be provided as soon as vote goes live]

Please take a look at [the discourse thread for the supply vote](https://discourse.teia.art/t/vote-discussion-total-supply-of-tea-dao-tokens/625) for a more detailed vote description and to discuss the vote there

### Example: 6 Million Tokens total supply

For 6 Million Tokens as  total supply, 5 Million will be reserved for activity with 8% of those 5 Million allocated to contribution rewards as laid out above, resulting in the following split:
...
- 350k Tokens (~ 6%) will be locked in the treasury for future drops.
...
The Teia DAO tokens only purpose is voting on operational decisions for Teia as registered non-profit DAO LLC. The Tokens define who is member of the DAO. Due to the nature of the RMI DAO LLC legal construct, membership of the LLC is defined by ledger addresses on the blockchain. This means, anyone who holds Teia DAO Tokens is considered a part of the DAO. 

Due to the limited liability legal setup Teia has incoprporated under, the LLC acts as a legal wrapper, so individual members wont be held legally responbsible for actions of the DAO (unless they are commiting a crime).

#### Why do i need to claim my tokens? Why are they not airdopped?
...
Wallets that have interacted with the Hic et Nunc and/or Tezos smart contracts for longer than 14 days are eligible to claim activity rewards. Any wallet that had an hDAO balance during the snapshot block can claim an equal amount of Tezos DAO Tokens.

**Wallets that are excluded from the drop are:**

- Wallets on the restricted list for breaking the marketplace rules.
- Wallets that have obviously engaged in wash trading to receive hDAO rewards during the hDAO distribution period wont receive Teias Token rewards for their hDAO and activity parameters 7 and 8.
...
The exact claiming period is not yet decided at this point. but a soft consensus seems to form within the DAO working group that something around 1-3 month seem reasonable. 
...
#### When will we be able to vote in the DAO? Is there a roadmap?
...
If you have been active in the Teia/hicetnunc ecosystem with multiple wallets, you will need to claim your Teia DAO Tokens with each of those wallets to receive all the Tokens you are eligible to.

If you had you minting collected activity distributed you could get a little more or less tokens than if you had the same activity on one wallet depending of the case. 
...
#### Is my activity on Objktcom/Versuum/Hicaf being counted, too?
...
#### How is the contribution level defined?
...
- TH: GitHub contributors to Teia with less than 5 commits (lvl1)
- TI: smaller donations to teia between 100 and 1k tezos
