...
As of today, no member needs to undergo KYC (Know Your Customer) just because they hold Teia DAO Tokens. According to the jurisdiction Teia falls under (Marshall Islands), anyone who holds 10% or more voting power in a DAO LLC must provide KYC. However, the smart contracts have a limit of tokens a wallet can hold implemented, which means no wallet can hold more than, for example, 5% (exact amount not decided yet) of overall Tokens. There will be exceptions to that rule for addresses like the treasury that need to hold more tokens but can’t be used to vote. So if someone for whatever reason gets hold of 10% or more voting power, they will have to provide KYC information to Teia. But as laid out above, we dont expect this to happen.
